...
In the decision tree plot, we start at the root node (depth 0):
  At the top, it is the overall probability of participants that are in A. 28% from classe A are classified as A (this is also a prevalance in statistics summary), 19% from B are classified as    A, 17% from C are classified as A, 16% from D are classified as A and 18% are classified as A. The node asks whether roll belt is less than 1.1 or not. If yes, then we go down to the root's     left child node. Further, the node asks if pitch forearm is less than -1.6. If yes, it classifies the classe as A. We keep on going to understand what impacts the likelihood of falling into     one of the given classe.
...
Second model: Random Forest
  Random Forest is the ensemble of decision trees. It combines multiple decision trees to get more accurate predictions. Random forest choses the predictors at random and takes the outputs of     multiple trees to make a decision. Think about decision trees and random forests as individual work and group work. 
...
From the model fit plot, we can sse that as the number of trees increases, the error decreases
...
The lowest OOB error is achieved by using 10 randomly chosen predictor at each split when building the trees which is default in the function above.
...
From the results above, SVM performed worse than randomforest and gradient boosted trees but still better than decision tree. The accuracy is only 77.98%

Let's combine two worst performing model and create a new model out of that to see whether it performs better than all of the model we have trained so far.
...
```{r EmsembleMethod}
comb = data.frame(prediction3, prediction4, classe = as.factor(testing$classe))
...
We can see from the results above that the combined method has much less error. 
